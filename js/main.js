document.addEventListener('DOMContentLoaded', function () {
  ...
  /* Background videos (gate + hero) — deferred a beat past the very first paint so
     they don't compete for bandwidth with the critical CSS/HTML on a slow
     connection (the poster image gives an instant visual in the meantime).
     Deliberately NOT tied to window "load": that event also waits for third-party
     scripts (e.g. the Instagram embed below) to finish, which can hang for a long
     time on a slow connection and would drag the video's start — and therefore
     the LCP metric — down with it. requestIdleCallback (with a capped timeout)
  ...
  whenIdle(function () { startVideo('gate-video'); });
  if (gateWasOpen) {
    document.addEventListener('mi:visitor-chosen', function () { whenIdle(function () { startVideo('hero-video'); }); }, { once: true });
  } else {
    whenIdle(function () { startVideo('hero-video'); });
  }
